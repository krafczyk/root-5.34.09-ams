...
void TPad::PaintFillArea3D(Int_t n, Float_t *p1, Option_t * opt)
{
...
//*-*- convert from 3-D to 2-D pad coordinate system
   ...
   for(int k=0;k<n;k++){
    ...
    fView->WCtoNDC(temp, out);
    xpad[k]=out[0];
    ypad[k]=out[1];
   }
   PaintFillArea(n,xpad,ypad,opt);
   delete[] xpad;
   delete[] ypad;
}

void TPad::PaintLine(Double_t x1, Double_t y1, Double_t x2, Double_t y2, Color_t color)
{
   // Paint line in CurrentPad World coordinates.
   ...
   if (!gPad->IsBatch()) {
      ...
      Color_t csave=gVirtualX->GetLineColor();
      gVirtualX->SetLineColor(color);
      gVirtualX->DrawLine(px1, py1, px2, py2);
      gVirtualX->SetLineColor(csave);
   }

   if (gVirtualPS) {
      Color_t csave=gVirtualPS->GetLineColor();
      gVirtualPS->SetLineColor(color);
      gVirtualPS->DrawPS(2, x, y);
      gVirtualPS->SetLineColor(csave);
   }
