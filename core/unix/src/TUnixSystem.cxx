...
static void SetRootSys()
{
   ...
   if (dladdr(addr, &info) && info.dli_fname && info.dli_fname[0]) {
      ...
      if (!realpath(info.dli_fname, respath)) {
         ...
         if(!gSystem->Getenv("ROOTSYS"))gSystem->Setenv("ROOTSYS", gSystem->DirName(rs));
      }
   }
#else
   return;
...
const char *TUnixSystem::UnixHomedirectory(const char *name)
{
   ...
   struct passwd *pw;
 if(getenv("HOME")){
     strcpy(mydir,getenv("HOME"));
//     cout <<" TUnixSystem::UnixHomedirectory-I-HomeDirHacked "<<mydir<<endl;
    return mydir;
   }
   else{
    cout<<"TUnixSystem::UnixHomedirectory-I-No $HOME VarDefinedWillProcess getpwuid()"<<endl;
    }
