...
void TGraphPainter::PaintGraphAsymmErrors(TGraph *theGraph, Option_t *option)
{
   ...
   Color_t color=theGraph->GetMarkerColor();
   Double_t cx  = 0;
   Double_t cy  = 0;
   if (mark >= 20 && mark <= 34) {
      cx = cxx[mark-20];
   ...
   for (Int_t i=0;i<theNpoints;i++) {
      ...
      if (xl1 > xl2) {
         if (arrowOpt) {
            ...
            if (!brackets) gPad->PaintLine(xl1,y,xl2,y,color);
            if (endLines) {
               gPad->PaintLine(xl2,y-ty,xl2,y+ty,color);
               if (braticks) {
                  gPad->PaintLine(xl2,y-ty,xl2+tx,y-ty,color);
                  gPad->PaintLine(xl2,y+ty,xl2+tx,y+ty,color);
               }
            }
         }
      ...
      if (xr1 < xr2) {
         if (arrowOpt) {
            ...
            if (!brackets) gPad->PaintLine(xr1,y,xr2,y,color);
            if (endLines) {
               gPad->PaintLine(xr2,y-ty,xr2,y+ty,color);
               if (braticks) {
                  gPad->PaintLine(xr2,y-ty,xr2-tx,y-ty,color);
                  gPad->PaintLine(xr2,y+ty,xr2-tx,y+ty,color);
               }
            }
         }
      ...
      if (yup2 > yup1) {
         if (arrowOpt) {
            ...
            if (!brackets) gPad->PaintLine(x,yup1,x,yup2,color);
            if (endLines) {
               gPad->PaintLine(x-tx,yup2,x+tx,yup2,color);
               if (braticks) {
                  gPad->PaintLine(x-tx,yup2,x-tx,yup2-ty,color);
                  gPad->PaintLine(x+tx,yup2,x+tx,yup2-ty,color);
               }
            }
         }
      ...
      if (ylow2 < ylow1) {
         if (arrowOpt) {
            ...
            if (!brackets) gPad->PaintLine(x,ylow1,x,ylow2,color);
            if (endLines) {
               gPad->PaintLine(x-tx,ylow2,x+tx,ylow2,color);
               if (braticks) {
                  gPad->PaintLine(x-tx,ylow2,x-tx,ylow2-ty,color);
                  gPad->PaintLine(x+tx,ylow2,x+tx,ylow2-ty,color);
               }
            }
         }
